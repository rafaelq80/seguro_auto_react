...
import { createSeguroColumns } from './SeguroColumns'

interface SeguroDataTableProps {
  seguros: Seguro[]
}

function SeguroDataTable({ seguros }: SeguroDataTableProps) {
  const navigate = useNavigate()

  const [sorting, setSorting] = useState<SortingState>([])
  const [globalFilter, setGlobalFilter] = useState('')

  const columns = createSeguroColumns()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
        ...
        >
          <PlusIcon size={20} />
          <span className="hidden sm:inline">Novo Seguro</span>
                ...
                return (
                  <div
                    ...
                    className={`${colSpan} flex items-center px-4 py-4 text-sm
                      ${index === 0 || index === 6 ? 'justify-center' : 'justify-start'}`}
                  >
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => navigate(`/atualizarseguro/${seguro.id}`)}
                ...
                <button
                  onClick={() => navigate(`/deletarseguro/${seguro.id}`)}
...
export default SeguroDataTable